...
export const getScanningCountByBus = async (req, res) => {
  try {
    ...
    if (!date) {
      return res
        .status(400)
        .send({ error: "Query Parameter date is required" });
    }
    if (!bus) {
...
export const registerAdmin = async (req, res) => {
  try {
    const { firstname, lastname, username, password, phoneNumber, role } =
      req.body;
    ...
    if (!isUserExists) {
      ...
      await admin
        ...
        .catch((err) => {
          return res
            .status(500)
            .send({ ErrorMessage: "Internal Server Error", error: err });
        });
    } else {
...
export const adminLogin = async (req, res, next) => {
  try {
    ...
    return res
      .status(500)
      .send({ ErrorMessage: "Internal Server Error", error: error });
  }
};
...
export const getTodayData = async (req, res) => {
  try {
    const result = await ScanData.aggregate([
      // Match documents with today's date
      { $match: { date: today } },
      ...
      // Group by busNumber and cityName, counting the number of students
      {
        $group: {
      ...
      // Project to format the output as required
      {
        $project: {
          _id: 0,
          ...
          studentCount: 1,
          city: "$_id.city",
          date: today, // Include today's date in the result
        },
      },
    ]);
